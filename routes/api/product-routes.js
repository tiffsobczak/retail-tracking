...
router.post('/', (req, res) => {
  ...
  */
    Product.create(req.body).then((product) => { // if there's product tags, we need to create pairings to bulk create in the ProductTag model
...
// update product
router.put('/:id', (req, res) => { // update product data
    ...
    }).then((updatedProductTags) => res.json(updatedProductTags)).catch((err) => { // console.log(err);
        res.status(400).json(err);
    });
});
